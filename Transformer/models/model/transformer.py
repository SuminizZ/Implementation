...
class Transformer(nn.Module):
    def __init__(self, embed_dim, src_vocab_size, trg_vocab_size, src_pad_idx, trg_pad_idx, hidden_dim, num_heads, N, device):
    ...
    def forward(self, src, trg):
        src_pad_mask = self.make_pad_mask(src, src, self.src_pad_idx, self.src_pad_idx)
        trg_pad_mask = self.make_pad_mask(trg, trg, self.trg_pad_idx, self.trg_pad_idx)
        src_trg_pad_mask = self.make_pad_mask(src, trg, self.src_pad_idx, self.trg_pad_idx)
    ...
    def make_pad_mask(self, src, trg, src_pad_idx, trg_pad_idx):
        ...
        src = src.ne(src_pad_idx).unsqueeze(1).unsqueeze(2)       
        src = src.repeat(1, 1, trg_len, 1)         # n_batch, 1, trg_len, src_len
        
        trg = trg.ne(trg_pad_idx).unsqueeze(1).unsqueeze(3)    
